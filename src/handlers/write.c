...
BPF_HASH(write_tmp_buffer, u64, struct write_data_t);
...
TRACEPOINT_PROBE(syscalls, sys_enter_write) {
    ...
    fill_syscall_data(&data->syscall_data, "write");
    ...
    write_tmp_buffer.update(&temp_file_id, data);
    return 0;
}
...
TRACEPOINT_PROBE(syscalls, sys_exit_write) {
    ...
    struct write_data_t *data = write_tmp_buffer.lookup(&temp_file_id);
    if (data == NULL) {
        return 0;
    }
    ...
    write_tmp_buffer.delete(&temp_file_id);
    return 0;
}